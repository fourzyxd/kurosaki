...
╚════════
  `
  let but = [
  {urlButton: {displayText: 'Source Code ♨️',url: 'https://github.com/fourzyxd/kurosaki'}}, 
  {urlButton: {displayText: 'Website 🔗',url: 'https://lynk.id/kuroyama.com'}}, 
...
case 'sc': case 'sourcecode': case 'script': {
  m.reply('*Script Berasal Dari :* https://github.com/fourzyxd/kurosaki\n\nJangan Lupa Bintang nya!')
  }
  break
case 'owner': {
...
case 'leave': {
  if (!isOwner) return m.reply(mess.botOwner)
  await ichi.groupLeave(m.chat).then((res) => m.reply('Sayonara ðŸ‘‹\nSulit Di Kontrol Semoga Hari Kalian Mengont**"')).catch((err) => m.reply('Fitur Error ❎'))
  }
  break
case 'block': {
  if (!isOwner) return m.reply(mess.botOwner)
